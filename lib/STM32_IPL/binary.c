			...
			if (out->bpp == IMAGE_BPP_BINARY) {
				...
				if (!zero) {
					for (int y = 0, yy = img->h; y < yy; y++) {
						...
						for (int x = 0, xx = img->w; x < xx; x++) {
							rgb888_t pixel = ((!mask) || image_get_mask_pixel(mask, x, y)) ?
...
void imlib_invert(image_t *img)
{
    switch(img->bpp) {
        ...
        case IMAGE_BPP_RGB888: { // STM32IPL
			for (rgb888_t *start = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, 0),
		...
		case IMAGE_BPP_RGB888: { // STM32IPL
			...
			if (!mask) {
				for (int i = 0, j = IMAGE_RGB888_LINE_LEN(img); i < j; i++) {
					data[i].r &= ((rgb888_t*)other)[i].r;
					data[i].g &= ((rgb888_t*)other)[i].g;
					data[i].b &= ((rgb888_t*)other)[i].b;
				}
			} else {
				rgb888_t pixel888, pixel888_data, pixel888_other;
				for (int i = 0, j = img->w; i < j; i++) {
					if (image_get_mask_pixel(mask, i, line)) {
						//rgb888_t pixel888;
						pixel888_data = IMAGE_GET_RGB888_PIXEL_FAST(data, i);
						pixel888_other = IMAGE_GET_RGB888_PIXEL_FAST(((rgb888_t* ) other), i);
						pixel888.r = pixel888_data.r & pixel888_other.r;
						pixel888.g = pixel888_data.g & pixel888_other.g;
						pixel888.b = pixel888_data.b & pixel888_other.b;
						IMAGE_PUT_RGB888_PIXEL_FAST(data, i, pixel888);

						//IMAGE_PUT_RGB888_PIXEL_FAST(data, i,
						//		(IMAGE_GET_RGB888_PIXEL_FAST(data, i) & IMAGE_GET_RGB565_PIXEL_FAST(((rgb888_t *) other), i)));
					}
				}
			}
			break;
		...
		case IMAGE_BPP_RGB888: { // STM32IPL
				...
				for (int x = 0, xx = img->w; x < xx; x++) {
					...
					if (x > ksize && x < img->w - ksize && y >= ksize && y < img->h - ksize) { // faster
						for (int j = -ksize; j <= ksize; j++) {
							...
							acc -= pixel888.b & 1; //((pixel888.r << 16) | (pixel888.g << 8) | (pixel888.b)) & 1;
							pixel888 = IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x + ksize);
							acc += pixel888.b & 1; //((pixel888.r << 16) | (pixel888.g << 8) | (pixel888.b)) & 1;

							//acc -= IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x - ksize - 1) & 1; // already 0 or FFFF
							//acc += IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, x + ksize) & 1; // (pre-thresholded)
						...
						for (int j = -ksize; j <= ksize; j++) {
							...
							for (int k = -ksize; k <= ksize; k++) {
								...
								//acc += (IMAGE_GET_RGB888_PIXEL_FAST(k_row_ptr, IM_MIN(IM_MAX(x + k, 0), (img->w - 1))))
								//		& 1; // already 0 or FFFF
							}
						}
					}
					...
					if (!e_or_d) {
						// Preserve original pixel value... or clear it.
						if (acc < threshold) {
							pixel.r = 0; //(COLOR_RGB888_BINARY_MIN >> 16)& 0xFF;
							pixel.g = 0; //(COLOR_RGB888_BINARY_MIN >> 8) & 0xFF;
							pixel.b = 0; // C0LOR_RGB888_BINARY_MIN & 0xFF;
							IMAGE_PUT_RGB888_PIXEL_FAST(buf_row_ptr, x, pixel);
						}
					} else {
						// Preserve original pixel value... or set it.
						if (acc > threshold) {
							pixel.r = 255; //(COLOR_RGB888_BINARY_MAX >> 16)& 0xFF;
							pixel.g = 255; //(COLOR_RGB888_BINARY_MAX >> 8) & 0xFF;
							pixel.b = 255; // C0LOR_RGB888_BINARY_MAX & 0xFF;
							IMAGE_PUT_RGB888_PIXEL_FAST(buf_row_ptr, x, pixel);
						}
					}
				}
			...
			for (int y = IM_MAX(img->h - ksize, 0), yy = img->h; y < yy; y++) {
				memcpy(IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y),
						IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(&buf, (y % brows)), IMAGE_RGB888_LINE_LEN_BYTES(img));
			}
